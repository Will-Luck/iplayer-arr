...
- **Tier B CI pipeline: real-binary smoke + automated github mirror.** Two new jobs in `.gitea/workflows/ci.yml` extend the regression-safety framework with image-level coverage that Tier A's code-level tests cannot reach. The `smoke` job runs on a dedicated self-hosted runner (`gitea-runner-smoke`, label `57-smoke`) that builds the production image with no shortcuts, brings up a container with real ffmpeg, real `get_iplayer`, and real outbound BBC reachability, then curls every `/api/diag/*` endpoint against the live binary. The `mirror-to-github` job has `needs: [unit, diag-suite, smoke]` and pushes the gitea ref to the GitHub mirror only when all three gates are green — removing the manual `git push github main` step. Tier B catches the class of failure where the code is correct but the deployed binary is broken (the v1.5.5 ffmpeg-8 `kB→KiB` rename being the canonical example). PAT lives in 1Password as `gh-pat-iplayer-mirror` and is bound to the Gitea Actions secret `GH_PAT`. See `docs/testing.md` for the framework split between Tier A (code-level) and Tier B (image-level).
